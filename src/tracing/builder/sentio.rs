...
fn copy_stack(stack: &[U256], size: usize) -> Vec<U256> {
    stack[stack.len() - size..].to_vec()
}
...
mod tests {
    ...
    #[test]
    fn copy_stack_returns_only_requested_items() {
        let stack = [1, 2, 3, 4, 5].map(U256::from);

        assert_eq!(copy_stack(&stack, 3), stack[2..]);
    }
}
